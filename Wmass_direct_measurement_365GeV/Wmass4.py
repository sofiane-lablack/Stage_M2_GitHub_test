...
#		partchargees=['11','13','130','211']
	...
	energypartchargeesjet4= from_tree.get_reco_jet4_11_e()+from_tree.get_reco_jet4_13_e()+from_tree.get_reco_jet4_211_e()
		#cut energy part chargees < 0.95 * energy jet
#	if energypartchargeesjet1 > 0.95 * energyjet1 or energypartchargeesjet2 > 0.95 * energyjet2 or energypartchargeesjet3 > 0.95 * energyjet3 or energypartchargeesjet4 > 0.95 * energyjet4:
#		continue
...
##		if energy_part_chargees_jet_number < 0.95*energyjet+L[jetnumber]:		
	    #continue
	...
	#if from_tree_get_jet1_e()<0.95.*energypartchargeesjet
		#continue
	...
	#if energy_part_chargees_jet_"number"<0.95.*energy_part_chargees_jet_"number"
	#	continue
...
def main():
    ...
    # Analysis
    histdict = gethists()
    filename = "/afs/cern.ch/user/s/slablack/fcc/heppy/ee_ZZ_allBkg_162_50K_ee_kt_SM.root"
    ...
    # output file
    outdir = "/afs/cern.ch/user/s/slablack/fcc/output_wmass_diWmasscut/13/output_ee_ZZ_allBkg_162_diWmasscut_50K_ee_kt_SM"
